...
function filterByAmenity(diffAmenity, amenity) {
  return diffAmenity.amenity.toLowerCase() === amenity.toLowerCase();
}

app.get('/hotels/filter/amenity', (req, res) => {
  let amenity = req.query.amenity;
  let result = hotels.filter((diffAmenity) =>
    filterByAmenity(diffAmenity, amenity)
  );
  res.json({ hotels: result });
});

// Endpoint 5: Filter the hotels based on the selected Country.
function filterByCountry(diffCountry, country) {
  return diffCountry.country.toLowerCase() === country.toLowerCase();
}

app.get('/hotels/filter/country', (req, res) => {
  let country = req.query.country;
  let result = hotels.filter((diffCountry) =>
    filterByCountry(diffCountry, country)
  );
  res.json({ hotels: result });
});

// Endpoint 6: Filter the hotels based on the selected Category.
function filterByCategory(diffCategory, category) {
  return diffCategory.category.toLowerCase() === category.toLowerCase();
}

app.get('/hotels/filter/category', (req, res) => {
  let category = req.query.category;
  let result = hotels.filter((diffCategory) =>
    filterByCategory(diffCategory, category)
  );
  res.json({ hotels: result });
});
